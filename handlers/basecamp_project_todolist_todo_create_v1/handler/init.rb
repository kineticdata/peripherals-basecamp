...
class BasecampProjectTodolistTodoCreateV1
  ...
  def execute()
    ...
    if not due_at =~ /\d\d\d\d-\d\d-\d\d/ and due_at =~ /\S/
      raise "Invalid date format, must be yyyy-mm-dd, or no characters. You entered [#{due_at}]."
    else
      due_at = nil
    end
    ...
    # -or-
    # exactly one completed todolist
    if check_alists.length > 0 and check_clists.length > 0
      ...
      raise "There is no todolist with the name, #{todolist_name}, for the project, #{project_name}."
    elsif check_alists.length == 1 and check_clists.length == 0
      list = check_alists[0]
      wrap.create_todo(project['id'],list['id'], content, due_at)
