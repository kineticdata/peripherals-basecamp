...
class BasecampProjectDeleteV1
  ...
  def execute()
    ...
    # There must be exactly one project with this name
    if project.length < 1
      raise "There is no project with the name, #{project_name}."
    elsif project.length > 1
      raise "There are more than one project with the name #{project_name}."
    else
      project = project[0]
      # Create project if one with the same name does not already exist.
      wrap.delete_project(project['id'])
    end
